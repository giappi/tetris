...
GameStateCenter::GameStateCenter()
{
    ...
    this->sdl_Window = SDL_CreateWindow(u8"Người đan chữ xếp hình", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 800, 360, SDL_WINDOW_SHOWN);
    if (this->sdl_Window == NULL)
    {
        __printf__("Window could not be created! SDL_Error: %s\n", SDL_GetError());
    }
    else
    {
